...
#include <../GeographicLib/include/AlbersEqualArea.hpp>
...
namespace GeographicLib {
  ...
  void AlbersEqualArea::Init(real sphi1, real cphi1,
    ...
    if (polar || tphi1 == tphi2) {
      ...
      for (int i = 0; i < 2*numit0_ || GEOGRAPHICLIB_PANIC; ++i) {
        ...
        // Now
        // qZ-q0 = (1+e2_*sphi0)*(1-sphi0)/(1-e2_*sphi0^2) +
        //         (1-e2_)*atanhee((1-sphi0)/(1-e2_*sphi0))
        // In limit sphi0 -> 1, qZ-q0 -> 2*(1-sphi0)/(1-e2_), so wrte
        // qZ-q0 = 2*(1-sphi0)/(1-e2_) + A + B
        // A = (1-sphi0)*( (1+e2_*sphi0)/(1-e2_*sphi0^2) - (1+e2_)/(1-e2_) )
        //   = -e2_ *(1-sphi0)^2 * (2+(1+e2_)*sphi0) / ((1-e2_)*(1-e2_*sphi0^2))
        // B = (1-e2_)*atanhee((1-sphi0)/(1-e2_*sphi0)) - (1-sphi0)
        //   = (1-sphi0)*(1-e2_)/(1-e2_*sphi0)*
        //     ((atanhee(x)/x-1) - e2_*(1-sphi0)/(1-e2_))
        // x = (1-sphi0)/(1-e2_*sphi0), atanhee(x)/x = atanh(e*x)/(e*x)
        //
        // 1 - scbet0^2 * sphi0 * (qZ-q0)
        //   = 1 - scbet0^2 * sphi0 * (2*(1-sphi0)/(1-e2_) + A + B)
        //   = D - scbet0^2 * sphi0 * (A + B)
        // D = 1 - scbet0^2 * sphi0 * 2*(1-sphi0)/(1-e2_)
        //   = (1-sphi0)*(1-e2_*(1+2*sphi0*(1+sphi0)))/((1-e2_)*(1+sphi0))
        // dD/dsphi0 = -2*(1-e2_*sphi0^2*(2*sphi0+3))/((1-e2_)*(1+sphi0)^2)
        // d(A+B)/dsphi0 = 2*(1-sphi0^2)*e2_*(2-e2_*(1+sphi0^2))/
        //                 ((1-e2_)*(1-e2_*sphi0^2)^2)
  ...
  Math::real AlbersEqualArea::txif(real tphi) const {
    // sxi = ( sphi/(1-e2_*sphi^2) + atanhee(sphi) ) /
    //       ( 1/(1-e2_) + atanhee(1) )
    //
    // txi = ( sphi/(1-e2_*sphi^2) + atanhee(sphi) ) /
    //       sqrt( ( (1+e2_*sphi)*(1-sphi)/( (1-e2_*sphi^2) * (1-e2_) ) +
    //               atanhee((1-sphi)/(1-e2_*sphi)) ) *
    //             ( (1-e2_*sphi)*(1+sphi)/( (1-e2_*sphi^2) * (1-e2_) ) +
    //               atanhee((1+sphi)/(1+e2_*sphi)) ) )
    //     = ( tphi/(1-e2_*sphi^2) + atanhee(sphi, e2_)/cphi ) /
    //       sqrt(
    //       ( (1+e2_*sphi)/( (1-e2_*sphi^2) * (1-e2_) ) + Datanhee(1,  sphi)  ) *
    //       ( (1-e2_*sphi)/( (1-e2_*sphi^2) * (1-e2_) ) + Datanhee(1, -sphi)  ) )
    //
    ...
    real
      ...
      sphi = tphi * cphi,
      es1 = _e2 * sphi,
      es2m1 = 1 - es1 * sphi,   // 1 - e2_ * sphi^2
      es2m1a = _e2m * es2m1;    // (1 - e2_ * sphi^2) * (1 - e2_)
  ...
  Math::real AlbersEqualArea::DDatanhee1(real x, real y) const {
    ...
    // In the limit x,y -> 1,
    //
    //   DDatanhee -> e2_/(1-e2_)^2 = sum(l * e2_^l, l, 1, inf)
    //
    // Use if e2_ is sufficiently small.
    real s = 0;
    ...
    while (true) {
      ...
      t = y * t + z; c += t; z *= x;
      k += 2; en *= _e2;
      // Here en[l] = e2_^l, k[l] = 2*l + 1,
      // c[l] = sum( x^i * y^j; i >= 0, j >= 0, i+j < 2*l) / (2*l + 1)
      // Taylor expansion is
      // s = sum( c[l] * e2_^l, l, 1, N)
      real ds = en * c / k;
      s += ds;
  ...
  Math::real AlbersEqualArea::DDatanhee2(real x, real y) const {
    ...
    //
    // Leading (m = 0) term is e2_ / (1 - e2_)^2
    //
    // Magnitude of mth term relative to the leading term scales as
    //
    //   2*(2*e/(1-e2_)*dx)^m
    //
    // So use series if (2*e/(1-e2_)*dx) is sufficiently small
    ...
    for (int m = 1; ; ++m) {
      ...
      ee /= -_e2m;
      if (m % 2 == 0) ee *= _e2;
      // Now ee = (-1)^m * e2_^(floor(m/2)+1) / (1-e2_)^(m+2)
      int kmax = (m+1)/2;
