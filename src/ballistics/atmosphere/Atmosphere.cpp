...
namespace Ballistics::Atmosphere
{
    ...
    [[nodiscard]] scalar Atmosphere::calcKp(const Ballistics::integer kp) const noexcept
    {
        scalar val = 0.;
        if (kp % 10 == 0)
        {
    ...
    [[nodiscard]] Indexes Atmosphere::interpolate(const Ballistics::scalar jd) const noexcept
    {
        Indexes indexes{};
    ...
    [[nodiscard]] integer Atmosphere::calcNumEighthPartOfDay(const Ballistics::scalar jd) const
    {
        const auto timeUTC = Time::Time<Time::Scale::UTC>({jd, 0.});
        const auto timeInMidnight = calcTimeInMidnight(timeUTC);
        const scalar hours = (jd - static_cast<scalar>(timeInMidnight.getDay()) - timeInMidnight.getPart()) * HOURS_IN_DAY;
    ...
    {
        const auto timeInMidnight = calcTimeInMidnight(timeUTC);
        const auto timeUT1 = converter_.convert<Time::Scale::UTC, Time::Scale::UT1>(timeInMidnight);
        const auto timeTT = converter_.convert<Time::Scale::UTC, Time::Scale::TT>(timeInMidnight);
        return iauGmst06(timeUT1.getDay(), timeUT1.getPart(), timeTT.getDay(), timeTT.getPart());
    }

    [[nodiscard]] vector3d Atmosphere::calcSunPosition(const Time::Time<Time::Scale::UTC> &t) const
    {
        const auto timeTDB = converter_.convert<Time::Scale::UTC, Time::Scale::TDB>(t);
        return ephemeris_.calcCoordinates(timeTDB, 11, 3);
    }
    ...
    [[nodiscard]] scalar Atmosphere::getDensity(const vector3d &r,
        ...
        const Indexes indexes = interpolate(static_cast<scalar>(t.getDay()) + t.getPart());
